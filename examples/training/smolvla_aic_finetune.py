...
"""Launch SmolVLA finetuning for `slobot/aic`.

SmolVLA is a VLA policy, so task conditioning comes from the LeRobot dataset's
natural-language `task` field. This helper generates a SmolVLA config where:

1. the text instruction is read from `meta/tasks.parquet` through LeRobot's
   standard `task` field and tokenized by the SmolVLA preprocessor;
2. `observation.state` is built by the SmolVLA preprocessor from normalized
...
DEFAULT_SMOLVLA_TASK_ID_KEY = "none"


def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description=(
            "Generate a dataset-aware SmolVLA finetuning config for slobot/aic and run lerobot-train."
        )
    )
    parser.add_argument(
        "--dataset-repo-id",
    ...
    parser.add_argument(
        ...
        help="Comma-separated camera feature keys to expose to SmolVLA. Use 'all' to keep every camera.",
    )
    parser.add_argument(
        "--task-id-key",
        default=DEFAULT_SMOLVLA_TASK_ID_KEY,
        help=(
            "Optional task-id observation key(s) to append to observation.state. "
            "SmolVLA uses the dataset's natural-language task instruction by default, so this defaults "
    ...
    parser.add_argument(
        "--job-name",
        default=None,
        help="Training job name. Defaults to smolvla_<dataset_repo_id>.",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
    ...
    parser.add_argument(
        ...
        help="SmolVLA checkpoint or Hub repo to fine-tune from. Use 'none' to train from scratch.",
    )
    parser.add_argument("--policy-device", default="cuda", help="Torch device to pass to SmolVLA.")
    parser.add_argument(
        "--normalization",
        default="mean_std",
    ...
    parser.add_argument("--steps", type=int, default=20_000, help="Number of finetuning steps.")
    parser.add_argument("--num-workers", type=int, default=4, help="Dataloader worker count.")
    parser.add_argument("--log-freq", type=int, default=50, help="Logging frequency in steps.")
    parser.add_argument("--save-freq", type=int, default=2_000, help="Checkpoint save frequency in steps.")
    ...
    parser.add_argument("--wandb-project", default="lerobot-smolvla-aic", help="Weights & Biases project name.")
    parser.add_argument("--wandb-entity", default="slobot", help="Weights & Biases entity.")
...
def build_train_config(
    ...
    job_name = args.job_name or f"smolvla_{sanitize_repo_id(args.dataset_repo_id)}"
    ...
    cfg = TrainPipelineConfig(
        ...
        policy=SmolVLAConfig(**policy_kwargs),
...
def main() -> int:
    ...
    resolved_task_id_keys = resolve_task_id_keys(
        dataset_meta.features,
        args.task_id_key,
        sample_keys=set(probe_sample.keys()) if probe_sample is not None else None,
    )
    ...
    (
        ...
        task_id_keys=resolved_task_id_keys,
        sample=probe_sample,
    ...
    print(f"W&B entity/project: {args.wandb_entity}/{args.wandb_project}")
