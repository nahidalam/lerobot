...
from lerobot.policies.memer.high_level_policy import HighLevelPolicy, build_visual_memory
...
class PI05MemerPolicy(PreTrainedPolicy):
    """PI05 Policy for LeRobot."""

    config_class = PI05MemerConfig
    name = "pi05_memer"
    ...
    def __init__(
        ...
        # Initialize high-level policy for MEMER if enabled
        self.high_level_policy = None
        if config.use_high_level_policy:
            logging.info("Initializing MEMER high-level policy...")
            self.high_level_policy = HighLevelPolicy(config)
    ...
    def reset(self):
        ...
        # MEMER-specific memory structures
        if self.high_level_policy is not None:
            # Recent frames window R_t (stores last N frames with their indices)
            self._recent_frames = deque(maxlen=self.config.recent_frames_window)
            self._recent_frame_indices = deque(maxlen=self.config.recent_frames_window)
            
            # Keyframes K_t (stores up to max_keyframes with their indices)
            self._keyframes = []  # List of (frame_index, frame_tensor) tuples
            ...
            # Step counter for high-level policy queries
            self._step_counter = 0
            
            # Global frame index (increments with each observation)
    ...
    def _update_memer_memory(self, batch: dict[str, Tensor], general_task: str):
        """
        Update MEMER memory structures with new observations.
        
        This handles:
        1. Adding new frames to recent frames window R_t
        2. Querying high-level policy every k steps to get subtask and candidate frames
        3. Building visual memory K_t from candidate frame lists
        ...
        if first_camera_key in batch:
            ...
            # Query high-level policy at specified intervals
            if self._step_counter % self.config.high_level_query_interval == 0:
                # Prepare recent frames and keyframes for high-level policy
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        If MEMER high-level policy is enabled, this will:
        1. Update memory with current observations
        2. Query high-level policy periodically for subtasks
        3. Use the subtask to condition the low-level policy
        """
        self.eval()

        # MEMER: Update memory and potentially get new subtask
        if self.high_level_policy is not None:
            # Get general task from batch (assumes task is provided)
            general_task = batch.get("task", [""])[0] if isinstance(batch.get("task"), list) else ""
            
            # Update MEMER memory structures
            self._update_memer_memory(batch, general_task)
            
            # Override task with subtask if available
            if self._current_subtask is not None:
                # Create a modified batch with the subtask
                batch = batch.copy()
                batch["task"] = [self._current_subtask]
